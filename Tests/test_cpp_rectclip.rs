...
fn test_rectclip_basic() {
    ...
    let sub = vec![clipper2_rust::make_path64(&[
        100, 100, 700, 100, 700, 500, 100, 500,
    ])];
    let sol = clipper2_rust::rect_clip_64(&rect, &sub);
    ...
    let sol = clipper2_rust::rect_clip_64(&rect2, &sub);
    assert!(sol.is_empty());

    // Triangle outside rect
    let sub = vec![clipper2_rust::make_path64(&[430, 290, 470, 330, 390, 330])];
    let sol = clipper2_rust::rect_clip_64(&rect2, &sub);
    assert!(sol.is_empty());

    // Another triangle outside rect
    let sub = vec![clipper2_rust::make_path64(&[450, 290, 480, 330, 450, 330])];
    let sol = clipper2_rust::rect_clip_64(&rect2, &sub);
    assert!(sol.is_empty());
    ...
    let sol = clipper2_rust::rect_clip_64(&rect3, &sub);
...
fn test_rectclip3_issue_637() {
    ...
    let solution = clipper2_rust::rect_clip_64(&r, &subject);
    assert_eq!(solution.len(), 1);
}
...
fn test_rectclip_orientation_issue_864() {
    ...
    let subject = clipper2_rust::make_path64(&[375, 1680, 1915, 4716, 5943, 586, 3987, 152]);
